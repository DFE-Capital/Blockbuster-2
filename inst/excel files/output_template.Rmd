...
backlog <- read.xlsx(file.path(params$path), 1) %>%
  mutate(`Year on year change` = backlog - c(0, backlog[-length(backlog)]))
...
prev <- repair_money + rebuild_money
```
...
knitr::kable(backlog %>%
               mutate(`Year on year change` = format_money(`Year on year change`),
                      ...
                      previous = prev),
             row.names = FALSE,
             col.names = c("Year", "Backlog", "Year on year change", "Previous year's investment in estate"))
```
