...
determined by a quantity surveyor through grades (states); N, A, B, C, D, E. The
...
made of many blocks, `urn`; omitted for anonymity), at the Local Authority (LA)
level (`lano`) and at the National level given suitable input data (filter
...
in one or more school blocks.  Individual component level information is stored
in an 'element' class object (a data.frame with strict conditions on columns)
while block-level information is held in a 'block' class object (another
...
<!-- vignette for extra detail (e.g. try typing `?Blockbuster` and `?test.block` -->
<!-- into the console).   -->

<!-- ### Input variables of each building component -->

<!-- Provided with this package is a fake dataset `test_data`. This has been -->
<!-- constructed using the Property Data Survey dataset, removing identifying -->
<!-- features (e.g the school Unique Reference Number), and randomizing the areas and -->
<!-- grades of all components. The Blockbuster package utilises filter methods built -->
<!-- on the [`dplyr`](https://cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html) package which are useful for handling this data.  Running the  -->
<!-- forecasting on the complete data can take a non-trivial amount of time so  -->
<!-- testing on smaller slices is advisable. The data is held in objects called -->
<!-- 'test.block' and 'test.element', which store block-level and component-level -->
<!-- information respectively. -->

<!-- The `areafy2` function was used on data queried from the Department for -->
<!-- Education Education Funding Authority Property Data Survey SQL data tables. This -->
<!-- provides estimates for the `unit_area` variable, a quantification of how much of -->
<!-- a building component there is). This is crucial for the DTMC methodology and for -->
<!-- quantifying the costs of repairs. -->

<!-- We start by taking a look at the component-level data in `test.element`. -->

<!-- ```{r} -->
<!-- data("test_data") -->
<!-- dplyr::glimpse(test.element) -->
<!-- ``` -->

<!-- The `buildingid` variable identifies the building, and the `elementid` is a key -->
<!-- that identifies the precise component type.  The `N`, `A`, `B`, `C`, `D`, `E` -->
<!-- columns show the proportion of the component `unit_area` that is at each -->
<!-- condition grade.  The `na`, `ab`, `bc`, `cd`, `de` columns contain the -->
<!-- deterioration rates between grades at each timestep.  This is stored in the data -->
<!-- to allow for the manual alteration of rates for individual components in -->
<!-- buildings should we identify strong local factors affecting transition rates in  -->
<!-- the future. Finally, the data also contains information regarding the cost per -->
<!-- unit area of repairing the component at each grade rate as well as the total -->
<!-- cost to repair all of a given grade at the appropriate proportion. -->

<!-- The `test.block` aggregates the component-level information by building. -->

<!-- ```{r} -->
<!-- dplyr::glimpse(test.block) -->
<!-- ``` -->

<!-- The `ratio` column contains the ratio between the total repair costs at grades -->
<!-- B, C, and D for a block, and the cost to rebuild the entire block.  This is used -->
<!-- to evaluate the cost-effectiveness of rebuilding decisions within the model. -->

<!-- ### The output variables compared to the input -->

<!-- The output of the `Blockbuster` function is a list.  Each list entry contains two -->
<!-- objects: the component level state and the block level state of dataframes. The  -->
<!-- number of list entries is equivalent to the number of years simulated plus one. -->
<!-- The output is of the custom class `blockbuster` (derivative to list). -->

<!-- Here we simulate the deterioration of one building through one year. We are -->
<!-- interested in the variables of a simulated PDS produced by our `Blockbuster` -->
<!-- model using default arguments.  Note that the `Blockbuster` functions saves -->
<!-- interim data in the `./output/` folder by default, although this behaviour can  -->
<!-- be turned off with the argument `save = FALSE`. -->

<!-- ```{r, warnings = FALSE} -->
<!-- #  output demonstration -->

<!-- # construct single block input -->
<!-- my_block <- test.element %>% -->
<!--   dplyr::filter(buildingid == 4472) -->

<!-- my_block_output <- Blockbuster(my_block, save = FALSE) -->
<!-- dplyr::glimpse(my_block_output) # this function makes the output nicer to read -->

<!-- ``` -->

<!-- The first list entry is the initial state and subsequent list entries are the  -->
<!-- model outputs for each timestep. Observe that the C.block.repair.cost in the -->
<!-- first list entry (which can be read with the command -->
<!-- `my_block_output[[1]]$block$C.block.repair.cost`) is -->
<!-- £XXXX. After deterioration we find it has -->
<!-- increased to £XXXX reflecting the  -->
<!-- deterioration of the block components. -->
