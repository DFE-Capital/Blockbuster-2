...
For convenience there are four useful functions for managing the output.  `pull_Element_Data` and `pull_Block_Data` can be used to pull out the element- and block-level data from the output. For large datasets (for example, the PDS dataset), R can hit memory limits when the `Blockbuster` function tries to construct the single output object from the saved interim files. `load_Block_Data` and `load_Element_Data` load the relevant outputs from the saved files.

We start by looking at the block-level output:

```{r, collapse = TRUE}
my_block <- pull_Block_Data(my_block)
glimpse(my_block)
```

Note that the `pull_Block_Data` combines the outputs for all years into one data.frame and adds a `year` column.


Observe that the C.block.repair.cost for the first block in the simulated data (which can be read with the command `my_output[[1]]$block$C.block.repair.cost[1]`) is
`r paste0("£", round(my_output[[1]]$block$C.block.repair.cost[1], 2))`. After one year of deterioration we find it has increased to `r paste0("£", round(my_output[[2]]$block$C.block.repair.cost[1], 2))` reflecting the
deterioration of the block components.

We use the `ggplot` package to produce some plots
