...
#                                 object.")
  ...
  if(!is.numeric(rebuild.money)) stop ("The rebuild.money argument must be numeric.")
  ...
  if(any(rebuild.money < 0)) stop("You have supplied negative values for the rebuild budget.")
  if(all(rebuild.money == 0)) message("There will be no rebuilding as rebuild.money is zero for all years.")

  if(!is.numeric(repair.money)) stop ("The repair.money argument must be numeric.")
  ...
  if(any(repair.money < 0)) stop("You have supplied negative values for the repair budget.")
  if(all(repair.money == 0)) message("There will be no repairing as repair.money is zero for all years.")
  if(!is.numeric(block.rebuild.cost)) stop("The unit block rebuild cost must be a positive number.")
  if(any(block.rebuild.cost <= 0)) stop("The unit block rebuild cost must be a positive number.")
  if(length(block.rebuild.cost) > 1) warning("Only the first value entered for block.rebuild.cost will be used.")
  if(!is.numeric(inflation)) stop("The inflation argument should be a single number or a vector with a value for each timestep.")
  ...
  if(any(inflation > 2)) message("For at least one year, inflation is set above 200%. Are you sure this is right.  A value of 1 represents 100% inflation, i.e. no change.")
